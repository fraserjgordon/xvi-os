...
static callsite_info findCallSiteInfo(const lsda_header_info& info, std::uintptr_t pc)
{
    ...
    while (static_cast<std::size_t>(calltable_ptr - calltable_start) < info.calltable_size)
    {
...
static const std::type_info* getCatchTypeInfo(const lsda_header_info& info, std::size_t index)
{
...
static std::optional<handler_info> findMatchingAction(const lsda_header_info& info, std::size_t action_id, const std::type_info* exception_type, void* exception_object)
{
    ...
    while (true)
    {
        ...
        else if (action.type_filter > 0)
        {
            ...
            auto catch_type = getCatchTypeInfo(info, static_cast<std::size_t>(action.type_filter));
            if (!catch_type)
            {
...
_Unwind_Reason_Code __gxx_personality_v0(int version, _Unwind_Action action, std::uint64_t exception_class, _Unwind_Exception* exception, _Unwind_Context* context)
{
    ...
    // Exception class is ignored - we re-extract it from the _Unwind_Exception when needed.
