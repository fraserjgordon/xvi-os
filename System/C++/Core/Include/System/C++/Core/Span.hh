#ifndef __SYSTEM_CXX_CORE_SPAN_H
#define __SYSTEM_CXX_CORE_SPAN_H
...
public:
    ...
    constexpr explicit(extent != dynamic_extent) span(_It __first, _End __last)
        : _M_data(to_address(__first)),
          _M_size(static_cast<size_t>(__last - __first))
    {
    }
    ...
    constexpr span(array<_T, _N>& __arr) noexcept
        requires (extent == dynamic_extent || extent == _N)
                  && is_convertible_v<remove_pointer_t<decltype(__XVI_STD_NS::data(__arr))>(*)[], element_type(*)[]>
        : _M_data(__XVI_STD_NS::data(__arr)),
          _M_size(_N)
    ...
    template <class _T, size_t _N>
    constexpr span(const array<_T, _N>& __arr) noexcept
        requires (extent == dynamic_extent || extent == _N)
                 && is_convertible_v<remove_pointer_t<decltype(__XVI_STD_NS::data(__arr))>(*)[], element_type(*)[]>
        : _M_data(__XVI_STD_NS::data(__arr)),
          _M_size(_N)
    ...
    template <class _R>
        requires ranges::contiguous_range<_R>
                 ...
                 && (ranges::borrowed_range<_R> || is_const_v<element_type>)
                 && (!__detail::__is_span_specialization<remove_cvref_t<_R>>::value)
                 && (!__detail::__is_array_specialization<remove_cvref_t<_R>>::value)
                 && (!is_array_v<remove_cvref_t<_R>>)
                 && is_convertible_v<remove_reference_t<ranges::range_reference_t<_R>>(*)[], element_type(*)[]>
...
#endif /* ifndef __SYSTEM_CXX_CORE_SPAN_H */
